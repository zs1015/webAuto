...
public class BaseCases {
     ...
     * @return
     */
    public WebElement getElement(Locator locator, Long timeOutSecond) {
        ...
        if ("id".equalsIgnoreCase(type)) {
            ...
            throw new RuntimeException("无此定位方式");
        }
        final By finalBy = by;
        WebElement element = wait.until(new ExpectedCondition<WebElement>() {
            public WebElement apply(WebDriver webDriver) {
                return driver.findElement(finalBy);
            }
        });
        return element;
    }
    ...
    public String getTipsNotNull(final Locator locator) {
        WebDriverWait wait = new WebDriverWait(driver, 5);
        String text = wait.until(new ExpectedCondition<String>() {
            public String apply(WebDriver webDriver) {
                WebElement element = getElement(locator);//获取元素
                //元素不为空，文本不为空，且长度大于0
                if (element != null && element.getText() != null && element.getText().length() > 0) {
                    return element.getText();
                }
